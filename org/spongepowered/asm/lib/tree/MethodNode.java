...
extends MethodVisitor {
    ...
    public List<ParameterNode> nameeters;
    ...
    public void visitParameter(String name, int access) {
        if (this.nameeters == null) {
            this.nameeters = new ArrayList<ParameterNode>(5);
        }
    ...
    public AnnotationVisitor visitParameterAnnotation(int nameeter, String desc, boolean visible) {
        ...
        if (visible) {
            if (this.visibleParameterAnnotations == null) {
                int names = Type.getArgumentTypes(this.desc).length;
                this.visibleParameterAnnotations = new List[names];
            }
            if (this.visibleParameterAnnotations[nameeter] == null) {
                this.visibleParameterAnnotations[nameeter] = new ArrayList<AnnotationNode>(1);
            }
            this.visibleParameterAnnotations[nameeter].add(an);
        } else {
            if (this.invisibleParameterAnnotations == null) {
                int names = Type.getArgumentTypes(this.desc).length;
                this.invisibleParameterAnnotations = new List[names];
            }
            if (this.invisibleParameterAnnotations[nameeter] == null) {
                this.invisibleParameterAnnotations[nameeter] = new ArrayList<AnnotationNode>(1);
            }
    ...
    public void accept(MethodVisitor mv) {
        ...
        for (i = 0; i < n; ++i) {
            ParameterNode nameeter = this.nameeters.get(i);
            mv.visitParameter(nameeter.name, nameeter.access);
        }
